...
const App = () => {
  ...
  const deletePreset = async (id, e) => {
    ...
    if (window.confirm("Delete this preset?")) {
      await deleteDoc(doc(db, 'artifacts', 'virtual-sign-sheet', 'presets', id));
    }
  };

  const saveAsPreset = async () => {
    const name = prompt("Name this preset:");
    if (name) await addDoc(collection(db, 'artifacts', 'virtual-sign-sheet', 'presets'), { ...sessionSettings, presetName: name });
  };
