...
import { User, Mail, Phone, ClipboardCheck, Printer, ChevronLeft, Lock, CheckCircle2, ArrowRight, RefreshCw, Folder, Briefcase, Settings, Plus, ImageIcon, X, Trash2, Smartphone, Save, Search, Download, Calendar, ShieldCheck, Eraser, Activity } from 'lucide-react';
...
const App = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', repId: '', invitedBy: '', securitiesLicense: '' });
  ...
  const performBackgroundSubmit = async (parsedData) => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', 'virtual-sign-sheet', 'public', 'data', 'signins'), {
        ...
        securitiesLicense: parsedData.securitiesLicense || 'N/A',
        invitedBy: 'N/A',
  ...
  const handleGuestSubmit = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'artifacts', 'virtual-sign-sheet', 'public', 'data', 'signins'), {
        ...
        securitiesLicense: (liveSession.reqSecuritiesLicense && isAgent && formData.securitiesLicense) ? formData.securitiesLicense : 'N/A',
      ...
      if (!isAgent || (isAgent && !rememberMe)) {
        setFormData({ name: '', email: '', phone: '', repId: '', invitedBy: '', securitiesLicense: '' });
        setIsAgent(false);
      }
      ...
      <div className={`modern-card ${isPreview ? 'preview-mode' : ''}`} style={isPreview ? { transform: 'scale(0.85)', transformOrigin: 'top center', margin: '0 auto', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', border: '8px solid #0f172a' } : {}}>
        ...
        {showSuccess && !isPreview ? (
          <div style={{textAlign: 'center', padding: '2rem'}}>
            ...
            <button onClick={() => { setShowSuccess(false); if(!rememberMe){ setFormData({ name: '', email: '', phone: '', repId: '', invitedBy: '', securitiesLicense: '' }); setIsAgent(false); } }} className="primary-button" style={{marginBottom: '1rem'}}>Done</button>
            {autoSignTriggered && (
              ...
              {s.allowAgent && isAgent && (
                <div style={{ animation: 'fadeIn 0.3s' }}>
                  ...
                  {s.reqSecuritiesLicense && (
                    <div className="input-group" style={{ animation: 'fadeIn 0.3s' }}>
                      <label className="input-label">Securities License</label>
                      <div className="input-wrapper"><ShieldCheck size={18} className="input-icon" /><input className="modern-input" required placeholder="License Number" value={formData.securitiesLicense} onChange={(e) => setFormData({...formData, securitiesLicense: e.target.value})} disabled={isPreview}/></div>
                    </div>
                  )}
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', gap: '1.5rem'}}>
                      ...
                      <div style={{ padding: '1rem', background: '#f8fafc', borderRadius: '12px' }}>
                        ...
                        <div style={{display: 'flex', gap: '1rem', marginTop: '1rem'}}>
                           <div style={{flex: 1}}>
                              <label style={{fontSize: '0.8rem', color: '#64748b'}}>Guest Tab Label</label>
                              <input className="modern-input" style={{padding: '0.5rem'}} value={builderSession.guestTabLabel} onChange={(e) => updateBuilder({ guestTabLabel: e.target.value })} />
                           </div>
                           <div style={{flex: 1}}>
                              <label style={{fontSize: '0.8rem', color: '#64748b'}}>Agent Tab Label</label>
                              <input className="modern-input" style={{padding: '0.5rem'}} value={builderSession.agentTabLabel} onChange={(e) => updateBuilder({ agentTabLabel: e.target.value })} />
                           </div>
                        </div>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '1rem', borderTop: '1px solid #e2e8f0', paddingTop: '1rem'}}>
                          ...
                          <label style={{display:'flex', gap:'0.5rem'}}><input type="checkbox" checked={builderSession.reqSecuritiesLicense} onChange={(e)=>updateBuilder({reqSecuritiesLicense: e.target.checked})}/> Securities License Field (Agents Only)</label>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div>
                          ...
                          <div style={{display: 'flex', gap: '1rem', marginTop: '0.5rem'}}>
                            <button onClick={() => fileInputRef1.current.click()} className="admin-toggle"><Plus size={14} /> Upload</button>
                            {builderSession.logo && <button onClick={() => updateBuilder({logo: ''})} className="admin-toggle" style={{color: '#ef4444'}}><X size={14}/></button>}
                        ...
                        <div>
                          ...
                          <div style={{display: 'flex', gap: '1rem', marginTop: '0.5rem'}}>
                            <button onClick={() => fileInputRef2.current.click()} className="admin-toggle"><Plus size={14} /> Upload</button>
                            {builderSession.logo2 && <button onClick={() => updateBuilder({logo2: ''})} className="admin-toggle" style={{color: '#ef4444'}}><X size={14}/></button>}
